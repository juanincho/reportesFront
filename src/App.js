...
function App() {
  ...
  const obtenerDatos = async () => {
    ...
    if (selectedValue === "opcion1") {
      valores = ["creado_el", "host", "estatus", "EstatusConciliadoConsolidado"];
      ...
      valores = ["creado_el", "payment_type", "total_impuestos", "customer_payment_status"];
      headersArray = ["Fecha", "Concepto", "Monto", "Estado"];
    }
  ...
  return (
    <div className="App bg-sky-500 flex h-full flex-row justify-center items-center">
      <div className='w-2/6 bg-gray-50 h-screen space-y-7 p-4 overflow-y-auto'>
      ...
      <div className='w-4/6 flex flex-col justify-center items-center bg-gradient-to-r from-sky-900 to-sky-700 h-screen overflow-y-scroll text-white'>
        {presionado && (
          <div className='w-full flex flex-col overflow-y-auto h-full p-4'>
            ...
            <h2>Pivot table</h2>
            <div className='overflow-y-scroll h-96 backdrop-blur-sm'>
              <PivotTableUI
                data={data}
